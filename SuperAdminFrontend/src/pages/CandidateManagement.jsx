...
import CandidateProfileModal from "../components/layout/CandidateProfileModal";
...
const CandidateManagement = () => {
  ...
  const filteredCandidates = candidates.filter((candidate) => {
    const matchesSearch =
      (candidate.name?.toLowerCase() || "").includes(
        searchTerm.toLowerCase()
      ) ||
      (candidate.candidate_state?.toLowerCase() || "").includes(
        searchTerm.toLowerCase()
      ) ||
      (candidate.party_name?.toLowerCase() || "").includes(
        searchTerm.toLowerCase()
      );

    const matchesState =
      stateFilter === "all" ||
      (candidate.candidate_state?.toLowerCase() || "") ===
        stateFilter.toLowerCase();
    const matchesParty =
      partyFilter === "all" ||
      (candidate.party_name?.toLowerCase() || "") === partyFilter.toLowerCase();
    // const matchesStatus =
    //   statusFilter === "all" ||
            ...
            <CardContent className="p-5 md:p-6 pt-0 flex-grow flex flex-col">
              <div className="space-y-3 mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm">
                    {candidate.candidate_city} , {candidate.candidate_district}{" "}
                  </span>
                </div>
                <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm">
                    {candidate.experience} experience • {candidate.previousWins}{" "}
                    wins
                  </span>
                </div>
                <div className="flex items-center gap-2">
              ...
              <div className="p-3 bg-muted/30 rounded-lg mb-4">
                <div className="flex items-center justify-between">
                  <h4 className="text-sm font-semibold">Manifesto:</h4>
                  {candidate.manifesto ? (
                    <a
